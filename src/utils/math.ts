...
export class Transform {
  static projectPoint(point: Vec3, center: Point2D, view: ViewState): Point2D {
  ...
  static rotatePoint(point: Vec3, rotation: Vec3): Vec3 {
    const { x, y, z } = point;
  ...
  static getNormalVector(points: Vec3[]): Vec3 {
  ...
  static isFaceVisible(points: Vec3[]): boolean {
    const normal = this.getNormalVector(points);
  ...
  static isPointBehind(point: Vec3, center: Vec3): boolean {
    const viewVector = {
      x: -0.7071, // cos(45°)
      y: -0.3827, // -sin(30°)
      z: -0.7071  // cos(45°)
