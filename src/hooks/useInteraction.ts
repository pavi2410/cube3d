    ...
    (e: React.MouseEvent<SVGElement>) => {
      if (e.button === 0) {
        ...
        if (selection.type === "none") {
          // Only pan if nothing is selected
          e.preventDefault();
          setState((prev) => ({
            ...prev,
            isPanning: true,
          ...
          // If something is selected, handle dragging
          setState((prev) => ({
            ...prev,
            isDragging: true,
            dragStart: { x: e.clientX, y: e.clientY },
          ...
          case "corner": {
            ...
            setObjectState((prev) => ({
              ...
              scale: {
                x: Math.max(
                  0.1,
                  state.dragStartObject.scale.x * (1 + dx * scaleScale),
                ),
                y: Math.max(
                  0.1,
                  state.dragStartObject.scale.y * (1 + dy * scaleScale),
                ),
                z: Math.max(
                  0.1,
                  state.dragStartObject.scale.z * (1 + (dx + dy) * scaleScale),
                ),
              },
            }));
            break;
      ...
      switch (e.key) {
        ...
        case "X":
          setObjectState((prev) => ({
            ...
            scale: {
              ...prev.scale,
              x: Math.max(0.1, prev.scale.x - scaleAmount),
            },
          }));
          break;
        case "y":
        ...
        case "Y":
          setObjectState((prev) => ({
            ...
            scale: {
              ...prev.scale,
              y: Math.max(0.1, prev.scale.y - scaleAmount),
            },
          }));
          break;
        case "z":
        ...
        case "Z":
          setObjectState((prev) => ({
            ...
            scale: {
              ...prev.scale,
              z: Math.max(0.1, prev.scale.z - scaleAmount),
            },
          }));
          break;
      }
