...
export const Normals = ({ position, rotation, scale, view, center }: NormalsProps) => {
  // Transform vertices
  ...
  return (
    <g>
      {CUBE_FACES.map((face, index) => {
        ...
        const start2D = Transform.projectPoint(faceCenter, center, view);
        const end2D = Transform.projectPoint(normalEnd, center, view);

        // Only show normal if face is visible
        if (Transform.isFaceVisible(vertices)) {
          return (
            <g key={`normal-${index}`}>
              <line
                x1={start2D.x}
